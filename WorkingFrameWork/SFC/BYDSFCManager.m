           ...
           faiureMessage:(NSString *)failMsg
{
    ...
    switch (sfcCheckType)
    {
        ...
        case e_SN_CHECK:
        {
            ...
            [urlString appendFormat:@"%@=%@&",SFC_TEST_STATION_ID,[NSString stringWithFormat:@"%@%@",_unit.stationID,self.station_id]];
        ...
        case e_LOG_CHECK:
        {
            ...
            {
                [urlString appendFormat:@"&p%d=%@",i+1,array[i]];
            }
...
#pragma mark---------删除数据
- (BOOL)UploadDataFromLocalFile:(NSString *)content
{
     NSMutableString* urlString = [NSMutableString stringWithFormat:@"http://%@:%@/handle.act?c=",_unit.MESServerIP, _unit.netPort];
    [urlString appendFormat:@"LOG&%@=%@&",SFC_TEST_STATION_NAME,_unit.stationName];
    [urlString appendFormat:@"%@=%@",SFC_TEST_STATION_ID,[NSString stringWithFormat:@"%@",_unit.stationID]];
    [urlString appendString:content];
    
    NSString  * url = [[NSString alloc]initWithString:urlString];
    NSLog(@"Upload Data with requestUrl:%@",url);
    return [self submit:url];
}
