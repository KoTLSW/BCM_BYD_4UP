...
@interface ConfigInstr ()
{
    ...
    NSMutableArray  *  arr_Instr;                       //e4980数组
...
- (void)windowDidLoad {
    [super windowDidLoad];
     ...
     arr_Instr = [[NSMutableArray alloc]initWithCapacity:10];
    
    
    NSArray  *  arr =[aglientTools getUsbArray];
    for (NSString * agilentString in arr)
    {
        if (![agilentString containsString:@"EW-FIX"])
        {
            [arr_Instr addObject:agilentString];
        }
    ...
    [PopButton_2987B addItemsWithTitles:arr_Instr];
    [PopButton_2987C addItemsWithTitles:arr_Instr];
    [PopButton_2987D addItemsWithTitles:arr_Instr];
    
    //4980
    [PopButton_4980A addItemsWithTitles:arr_Instr];
    [PopButton_4980B addItemsWithTitles:arr_Instr];
    [PopButton_4980C addItemsWithTitles:arr_Instr];
    [PopButton_4980D addItemsWithTitles:arr_Instr];
