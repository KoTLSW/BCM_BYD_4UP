...
@interface ServerWindow ()
{
    ...
    __weak IBOutlet NSTextField *readyTimes_TF;
...
- (void)windowDidLoad {
    ...
    [readyTimes_TF setStringValue:[[NSUserDefaults standardUserDefaults] objectForKey:@"PresentNum"]];
    [FixtureID_TF setStringValue:param.fixtureID];
...
- (IBAction)Click_Button:(id)sender {
    ...
    if ([UseName_TF.stringValue isEqualToString:@"Eastwin"]&&[PassWord_TF.stringValue isEqualToString:@"Eastwin123"]) {
        ...
        [dictionary setObject:FixtureID_TF.stringValue forKey:@"FixtureID"];
        [dictionary writeToFile:plistPath atomically:YES];
        
        //3.存储当前泡棉的测试次数
        [[NSUserDefaults standardUserDefaults] setObject:readyTimes_TF.stringValue forKey:@"PresentNum"];
        [[NSUserDefaults standardUserDefaults] synchronize];
        
        //3.退出程序
        if ([[readyTimes_TF stringValue]intValue]>=[param.FoamNum intValue]) {
            
            [ShowWrongMessagfe setStringValue:@"泡棉已测试次数设置有误，重新设置"];
        }
        else
        {
             exit(0);
