...
@interface ViewController()<NSTextFieldDelegate,TestActionDelegate>
{
    ...
    __weak IBOutlet NSTextField *Operator_TF;
...
//    });
    ...
    //***********************判断泡棉测试区***********************//
    int Forum_Num = [[[NSUserDefaults standardUserDefaults] objectForKey:kPresentNum] intValue];
    if (Forum_Num>=[param.FoamNum intValue]) {
...
#pragma mark-------------//index=104,C治具测试结束，发送指令信号灯
        if (fix_D_num == 104) { //扫描SN
            ...
            if (param.isDebug) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    [Status_TF setStringValue:@"fix_D_num=104,A测试结束，点亮灯"];
                });
...
-(void)compareSNToServerwithTextField:(NSTextField *)tf Index:(int)testIndex SnIndex:(int)snIndex
{
    ...
    if ([tf.stringValue length] == [num_PopButton.titleOfSelectedItem intValue]) {
        ...
        {
            if ([testStep StepSFC_CheckUploadSN:YES Option:@"isPassOrNot" sn:tf.stringValue testResult:nil startTime:startTime testArgument:nil]) {
...
-(void)updataFoam
{
    ...
    if (nulltest_button.hidden&&(action1.isShow||action2.isShow||action3.isShow||action4.isShow)) {
        ...
        [alert ShowCancelAlert:@"Rfix参考值变化较大，请重新空测" Window:[NSApp mainWindow]];
